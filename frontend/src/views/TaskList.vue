  ...
  >
    ...
    >
      ...
      <a-form-item :wrapper-col="{ offset: 3, span: 21 }" style="margin-top: 16px; margin-bottom: 0;">
        ...
        <div v-if="formState.convergence_enabled" style="margin-top:12px;padding:12px;background:var(--arl-bg-card);border-radius:6px;border:1px solid var(--arl-border-color);">
          <div style="font-size:13px;font-weight:500;color:var(--arl-text-color);margin-bottom:8px;">收敛参数</div>
          <a-row :gutter="[16,12]">
            <a-col :span="8">
              <span style="font-size:12px;color:var(--arl-text-secondary);">最大轮次</span><br/>
              <a-input-number v-model:value="formState.convergence_max_rounds" :min="1" :max="10" style="width:100%;" />
            </a-col>
            <a-col :span="8">
              <span style="font-size:12px;color:var(--arl-text-secondary);">最小新增数</span><br/>
              <a-input-number v-model:value="formState.convergence_min_new" :min="1" :max="1000" style="width:100%;" />
            </a-col>
            <a-col :span="8">
              <span style="font-size:12px;color:var(--arl-text-secondary);">新增占比阈值</span><br/>
              <a-select v-model:value="formState.convergence_ratio" style="width:100%;">
                <a-select-option value="0.01">1%</a-select-option>
                <a-select-option value="0.03">3%</a-select-option>
                <a-select-option value="0.05">5%</a-select-option>
                <a-select-option value="0.10">10%</a-select-option>
...
const defaultPlugins = {
  ...
  npoc_service_detection: false, js_analysis: false, convergence_enabled: false, convergence_max_rounds: 3, convergence_min_new: 5, convergence_ratio: "0.05"
};
