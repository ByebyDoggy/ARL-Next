<template>
  <div style="background-color: var(--arl-bg-layout); padding: 24px; min-height: calc(100vh - 64px);">
    ...
    <a-tabs v-model:activeKey="activeTab" type="card" class="arl-detail-tabs">
      ...
      <a-tab-pane key="js_report" :tab="query.task_id ? `JS analysis - ${queryCounts.js_report || 0}` : `JS analysis`"></a-tab-pane>
...
// 💡 修复：WIH (Web Info Hunter) Tab 配置
  js_report: {
    url: "/api/js_analysis/js_report/",
    searchFields: [{ label: "站点", key: "site" }],
    cols: [{ title: "序号", key: "index", width: 60 }, { title: "站点", dataIndex: "site", width: 250 }, { title: "API端点", dataIndex: "api_endpoints", width: 70 }, { title: "Source Map", dataIndex: "sourcemap_count", width: 70 }, { title: "评估", dataIndex: "assessment", width: 300 }]
  },
  wih: {
    url: '/wih/',
